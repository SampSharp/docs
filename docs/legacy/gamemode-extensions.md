...
- `LoadControllers`: within this method you can load all [controllers](/gamemode-controllers) provided by this extension
...
Handling callbacks within an extension works the same as handling them within a game mode, but instead of placing the methods in the game mode, they can be placed within the entry point class of the extension. For more information on callback handling, see [callbacks](/gamemode-callbacks).
